# [Q006] The Plate of the Deep-Form
**Status:** [ACTIVE]
**Scale:** [MASSIVE]
**Related Stratum:** [STRATUM: 2_ERODING_AGE]

## 1. The Directive (Narrative Goal)
Acquire or forge a set of full Plate Armor to achieve a high-tier defensive state (AC 21).

## 2. Intelligence & Leads
*   **Lead:** Plate costs 1,500 gp from a standard merchant. *Source: PHB*
*   **Lead:** Scrap can be refined at a 10-to-1 ratio. 375 lbs of steel scrap needed for a craft-reduction. *Source: House Rules*
*   **Lead:** The ruins of the North Cut contain old iron and steel, but it is "brittle." *Source: Kaelen / Harlen*

## 3. Execution (Sandbox Objectives)
*   **Phase 1: Resources & Design**
    *   **Objective A:** [OPEN] Secure a blueprint or design for Plate Armor.
    *   **Objective B:** [IN-PROGRESS] Accumulate 1,500 gp OR 375 lbs of Standard Steel scrap.
*   **Phase 2: The Forge**
    *   **Objective C:** [REQUIRES: Blueprint & Materials] Devote 100+ Labor Hours to the forge.

## 4. Resolution (If Applicable)
*   **Outcome:** [PENDING]

## 5. Rewards & Integration
*   **XP Awarded:** [UNLOGGED] [250 XP] -> `experience_log.md`
*   **Loot/Currency:** [UNLOGGED] [Plate Armor [MAT: STEEL] [QUAL: ??]] -> `inventory_log.md`
*   **Standing/SP:** [UNLOGGED] [+20 Harlen (if forged there)] -> `relationship_log.md`
