# Character State: Wael
<!-- SOURCE: pc_sheet -->

**Name:** Wael
**Race:** Mountain Dwarf
**Class:** Cleric (Forge Domain)
**Level:** 2
**Experience (XP):** 650 <!-- SOURCE: experience_log -->
**Alignment:** True Neutral
**Age:** Unknown (Looks ravaged by time)

## Vital Stats
**Hit Points (HP):** 16 / 16
**Armor Class (AC):** 19 (Chain Mail +1, Shield)
**Initiative:** +1
**Speed:** 25 ft.
**Passive Perception:** 12
**Passive Insight:** 14

## Ability Scores
*   **STR:** 15 (+2)
*   **DEX:** 13 (+1)
*   **CON:** 15 (+2)
*   **INT:** 11 (+0)
*   **WIS:** 15 (+2)
*   **CHA:** 10 (+0)

## Active Modifier Registry (HUD)
*Always check this block before rolling.*

| Trigger / Condition | Source | Effect on Rolls & Checks |
| :--- | :--- | :--- |
| **Lost Left Arm** | Permanent Injury | Disadvantage on two-handed tasks without a brace. Cannot use two-handed weapons. |
| **Scavenger's Anchor** | Prosthetic (Equipped) | Negates two-handed Disadvantage for Tools/Shields. **Disadvantage** on all Dexterity-based checks. **Disadvantage** on Charisma (Persuasion) vs non-artisans. Speed -5ft. |
| **The Mason's Blinder** | Gear (Equipped) | Negates Persuasion penalty from Maimed Eye. +1 bonus to saves vs non-magical blinding effects. |
| **Blessing of the Forge** | Active Spell | +1 AC (factored into Vital Stats). |
| **Dwarven Resilience** | Racial Trait | **Advantage** on poison saving throws. |
| **Stonecunning** | Racial Trait | Double proficiency bonus on INT (History) checks related to stonework. |

## Proficiencies & Languages
*   **Armor:** Light, Medium, Heavy, Shields.
*   **Weapons:** Simple Weapons, Battleaxe, Handaxe, Light Hammer, Warhammer.
*   **Tools:** Mason's Tools (Dwarven), Smith's Tools (Forge Domain), Herbalism Kit (Hermit).
*   **Languages:** Common, Dwarvish.
*   **Saving Throws:** Wisdom, Charisma.
*   **Skills:** Religion (+2), Medicine (+4), Insight (+4), History (+2).

## Features & Traits
*   **Discovery (Hermit):** You have had a unique revelation (the quiet power found in the cold).
*   **Spellcasting:** Divine spells using Wisdom.
*   **Blessing of the Forge:** 1/Long Rest. +1 bonus to nonmagical armor (AC) or simple/martial weapon (Attack/Damage) for 24h.
*   **Darkvision:** 60 ft.
*   **Dwarven Resilience:** Advantage on poison saves; resistance to poison damage.
*   **Stonecunning:** Double proficiency on INT (History) for stonework.
*   **Dwarven Armor Training:** Proficiency with light and medium armor.
*   **Channel Divinity (1/Rest):**
    *   **Artisan's Blessing:** 1-hour ritual to create a metal item (up to 100gp).
    *   **Turn Undead:** Standard feature.
*   **Permanent Injury:** Lost Left Arm. Cannot use shields or two-handed weapons.
*   **Permanent Injury:** Maimed (Left Eye). Disadvantage on Sight-Perception and Persuasion.

## Resource Tracker
*   **Spell Slots (1st Level):** [X] [X] [X]
*   **Hit Dice:** [X] [ ] (1d8)
*   **Blessing of the Forge:** [X] (Active on Chain Mail)
*   **Inspiration:** [Yes]

## Spellbook
*   **Spell Save DC:** 12
*   **Spell Attack Bonus:** +4
*   **Cantrips:** Guidance, Mending, Toll the Dead.
*   **Prepared Spells (4 Total):**
    *   **Domain:** Identify, Searing Smite (Cold-Shaped).
    *   **Daily:** Bane, Command, Healing Word, Guiding Bolt.

---

## Inventory & Gear
**Total Weight:** 222.0 lbs / 225.0 lbs <!-- SOURCE: inventory_log -->
**Currency:** 9 gp, 1 sp <!-- SOURCE: currency_log -->

### Equipped
| Item | Weight | Notes |
| :--- | :--- | :--- |
| Chain Mail (+1) [MAT: STEEL] | 55.0 | Blessed; Worn (AC 17) |
| Shield [MAT: WOOD] | 6.0 | Equipped via Anchor (AC +2) |
| Scavenger's Anchor [MAT: IRON] | 8.0 | Prosthetic; Reduces Speed by 5ft |
| Heavy Smith's Hammer [MAT: STEEL] | 10.0 | 1d8 Bludgeoning; Versatile (1d10) |
| The Mason's Blinder [MAT: LEATHER] | 0.1 | Anatomical eyepatch |
| Light Crossbow [MAT: WOOD] | 5.0 | 1d8 Piercing (80/320 ft.); Ammunition, Loading, Two-Handed* |
| Damp Blue-Green Stone Box | 2.0 | <!-- SOURCE: artifacts --> Focus (Shield Mount) |

### Stored (Backpack & Satchel)
| Item | Weight | Notes |
| :--- | :--- | :--- |
| Dark Black Shard | 5.0 | <!-- SOURCE: artifacts --> Inside Stone Box |
| Shield [MAT: WOOD] | 6.0 | Repaired; strapped to pack |
| Mason's Tools [MAT: IRON] | 8.0 | `atlas/crafting/tools.md` |
| Smith's Tools [MAT: IRON] | 8.0 | `atlas/crafting/tools.md` |
| Herbalism Kit [MAT: VARIOUS] | 3.0 | `atlas/crafting/tools.md` |
| Backpack | 5.0 | |
| Bedroll | 7.0 | |
| Mess kit | 1.0 | |
| Tinderbox | 1.0 | |
| 10 Torches | 10.0 | |
| 10 Days Rations (Plain) | 10.0 | (5 Days moved to Kaelen) |
| 5 Days Rations (Salt-Pork) | 0.0 | (Carried by Kaelen) |
| Full waterskin | 5.0 | |
| Hempen Rope (50ft) | 10.0 | |
| Berath's Journal | 1.0 | |
| Berath's Satchel | 2.0 | |
| Dark Metal Rods (4 total) [MAT: IRON] | 8.0 | `atlas/crafting/materials/` |
| Pouch of Rare Minerals | 5.0 | |
| Scroll case of notes | 1.0 | |
| Winter blanket | 3.0 | |
| Common clothes | 3.0 | |
| Crossbow bolts (20) | 1.5 | |
| Bag of Crystalline Shards [MAT: GLASS] | 1.0 | `atlas/crafting/materials/` |
| Steel Shard [MAT: STEEL] | 10.0 | (1 moved to Kaelen) |
| Deep-Frost Crystal | 0.1 | |
| Frost-Vein Lichen (0.5) | 0.1 | `atlas/crafting/materials/` |
| Steel Hardware [MAT: STEEL] | 0.0 | (Moved to Kaelen) |
| Blessed Oil (Mason's Rest) | 0.0 | (Consumed) |
| Heavy Masonry Pry-bar [MAT: IRON] | 10.0 | Gift: Harlen; Tool |
| Mason's Pitons (6) [MAT: IRON] | 0.0 | (Consumed) |
| Tallow & Beeswax (8/10 Uses) | 1.0 | `atlas/crafting/materials/` |
| Professional Leatherworker's Kit [MAT: STEEL] | 5.0 | Restored; `atlas/crafting/tools.md` |
| Steel Vice-Clamp [MAT: STEEL] | 0.0 | (Carried by Kaelen) |
| Sharpening Stone [MAT: STONE] | 2.0 | `atlas/crafting/tools.md` |
| Quality Travel Bread (2 Days) | 2.0 | Reward: Verminer's Request |
| Currency (4 gp, 1 sp) | 0.1 | <!-- SOURCE: currency_log --> |
