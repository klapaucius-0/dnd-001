# [Q015] Brittle Defense
**Status:** [COMPLETED]
**Scale:** [MINOR]
**Related Stratum:** [STRATUM: 2_ERODING_AGE]

## 1. The Directive (Narrative Goal)
Restore the town guard's frost-bitten spears to prevent a total failure of Oakhaven's defenses.

## 2. Intelligence & Leads
*   **Lead:** The North Cut gate is experiencing rapid "White-Frost" contamination on iron gear. *Source: Harlen*
*   **Lead:** The frost makes iron brittle, causing it to shatter under standard smithing. *Source: Harlen*

## 3. Execution (Sandbox Objectives)
*   **Phase 1: Restoration**
    *   **Objective A:** [COMPLETED] Purge and restore Spearhead #1 (1 LH / DC 15).
    *   **Objective B:** [COMPLETED] Purge and restore Spearhead #2 (1 LH / DC 15).
    *   **Objective C:** [COMPLETED] Purge and restore Spearhead #3 (1 LH / DC 15).

## 4. Resolution (If Applicable)
*   **Outcome:** [COMPLETED] Wael used Mending over 3 LH to restore the spears.

## 5. Rewards & Integration
*   **XP Awarded:** [LOGGED] [50 XP]
*   **Loot/Currency:** [LOGGED] [1x Cold-Iron Shard] -> `inventory_log.md`
*   **Standing/SP:** [LOGGED] [+5 SP (Harlen), +2 SP (Oakhaven)]
